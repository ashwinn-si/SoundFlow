...
[![Version 1.1.3](https://img.shields.io/badge/version-1.1.3-3D9E6B)](https://github.com/ashwinn-si/SoundFlow/releases)
...
- The output and input sliders now track the Mac's own volume both ways. Moving
  one moves the other, so the app can no longer show 19% while the Mac is at
  81%. Previously nothing told SoundFlow the volume had changed, so its slider
  sat at whatever it last read.
- Mute is part of that: the speaker icon reflects the system mute and toggles it.

### 1.1.2

- The playing indicator now flattens to a single line for an app you have muted
  or turned to zero. It used to pulse as though the app were still audible.
- Playback is picked up as it starts rather than seconds later. Apps that play
  through a helper process — Chrome, Arc, Spotify, Discord, Slack — were the
  worst affected, and the mixer could miss them entirely while the menu bar
  popover was open.

### 1.1.1

- New app icon: five faders at different levels, one of them turned down —
  the app's own accent wash and capsule shapes rather than a generic waveform.
